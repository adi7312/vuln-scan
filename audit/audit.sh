...
docker scout cves --format only-packages --output vuln_scan_image_audit.txt --exit-code ghcr.io/adi7312/vuln-scan:latest 
if [ $? -ne 2 ]; then
...
docker scout cves --epss --epss-score 0.2 --output vuln_scan_prob_analysis.txt --exit-code ghcr.io/adi7312/vuln-scan:latest
if [ $? -ne 2 ]; then
...
docker sbom ghcr.io/adi7312/vuln-scan:latest --output vuln_scan_image_sbom.txt
